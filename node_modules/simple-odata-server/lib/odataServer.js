...
var serviceMetadata = require('./serviceMetadata.js')
...
ODataServer.prototype._initializeRoutes = function () {
  var self = this
  this.router.get('/\$batch', function (req, res) {
    var result ="batch"
   
    return res.end(result)
  })
  this.router.get('/\$serviceMetadata', function (req, res) {
  	var result = serviceMetadata(self.cfg)
	return res.end(JSON.stringify(result))
  })
  this.router.get('/\$metadata', function (req, res) {
	
    var result = metadata(self.cfg)
    res.writeHead(200, self.cfg.addCorsToHeaders({'Content-Type': 'application/xml', 'DataServiceVersion': '2.0', 'OData-Version': '2.0'}))
    console.log(result)
    return res.end(result)
  })
...
if(req.params.collection == "CardConfiguration" || req.params.collection == "ReportConfigSet" || req.params.collection == "KPIConfigSet" || req.params.collection == "Variants")
	{
		console.log(req.body.Page_Id)
		DataAccess.verifyCreateAccess (req.cookies.session.userid, req.params.collection, req.body.Page_Id, function(access){
			if(access){
				req.body.CreatedBy = ObjectId(req.cookies.session.userid)
  ...
  this.router.put('/:collection\\(:id\\)', function (req, res) {
	console.log(req.params.id)
	DataAccess.verifyUpdateAccess (req.params.collection, req.cookies.session.userid, req.params.id, function(access){
		if(access)
  ...
  this.router.delete('/:collection\\(:id\\)', function (req, res) {
	if(req.params.collection == 'OVPPageConfig' || req.params.collection == 'Scenarios'){
		DataAccess.verifyDeleteAccess (req.cookies.session.userid, req.params.id, function(access){
			if(access)
